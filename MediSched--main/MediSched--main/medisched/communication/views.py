...
def video_call_room(request, call_id):
    """Video/audio call room - FIXED VERSION"""
    try:
    ...
    # Determine template based on user type - FIXED LOGIC
...
@login_required
def test_video_call(request):
    """Test page to verify video call setup"""
    context = {
        'agora_app_id': settings.AGORA_APP_ID,
        'agora_certificate': settings.AGORA_APP_CERTIFICATE if settings.AGORA_APP_CERTIFICATE else 'Not set',
        'token_builder_available': AGORA_TOKEN_BUILDER_AVAILABLE,
    }
    return render(request, 'communication/test_video_call.html', context)
# Add this function to communication/views.py
@login_required
def simple_video_call(request, appointment_id):
    """Simple Jitsi video call - EASIEST SOLUTION"""
    appointment = get_object_or_404(Appointment, id=appointment_id)
    ...
    room_name = f"medisched-appointment-{appointment_id}"
    
    # Determine user display name
    if hasattr(request.user, 'doctor_profile'):
        display_name = f"Dr. {request.user.get_full_name() or request.user.username}"
    else:
    ...
    return render(request, 'communication/simple_video_call.html', context)